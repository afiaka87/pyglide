...
from glide_finetune.glide_util import (add_cond_to_uncond, get_tokens_and_mask,
...
def glide_wds_loader(
    ...
    def filter_dataset_laion(item):
        ...
        if nsfw_filter and nsfw_rating in ["NSFW"]: return False

        original_height = float(metadata["original_height"])
        original_width = float(metadata["original_width"])
        if original_height < min_original_height or original_width < min_original_width: return False
        aspect_ratio = (original_width / original_height)
        if aspect_ratio < ar_lower or aspect_ratio > ar_upper: return False
        ...
        if any(slur.lower() in caption for slur in words_to_skip): return False
        return True
    ...
    if dataset_name == "laion": filtered_dataset = dataset.select(filter_dataset_laion)
    elif dataset_name == "alamy": filtered_dataset = dataset.select(filter_dataset_alamy)
    else: raise ValueError( f"Unknown dataset: {dataset_name}. Must be one of 'laion' or 'alamy'.")
    ...
    def preprocess_dataset(item):
        ...
        if not enable_text or random() < uncond_p:
            if len(cond_text) > 0: tokens, mask = add_cond_to_uncond(tokenizer, cond_text)
            else: tokens, mask = get_uncond_tokens_mask(tokenizer)
        else:
        ...
        if enable_upsample:
            upsample_pil_image = original_pil_image.resize(upsample_image_shape).convert("RGB")
